    ...
    export default {
        ...
        methods: {
            ...
            handleStatus(index, row) {
                this.$http.get('http://localhost:5000/models/' + this.models[index].id + '/result')
                    .then(
            ...
            handleLog(index, row) {
                this.$http.get('http://localhost:5000/models/' + this.models[index].id + '/log')
                    .then(
                        function(response) { // 正确回调
            ...
            handleTrain(index, row) {
                this.$confirm('确定要开始/重新训练吗?要花费较长时间', '提示', {
                    ...
                    this.$http.post('http://localhost:5000/models/' + this.models[index].id + '/train', {}, {
                        headers: {
            ...
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该数据集, 是否继续?', '提示', {
                    ...
                    this.$http.post('http://localhost:5000/models/' + this.models[index].id + '/delete', {}, {
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                            }
                        })
