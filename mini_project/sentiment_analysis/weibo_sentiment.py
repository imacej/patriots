...
datadir = 'data/weibo'
modeldir = 'model/weibo'
testdir = 'test'

# 加载训练文件
def loadfile():
  print("读取语料数据")
  angry=pd.read_csv(datadir + '/angry.txt', sep='\n',header=None)
  low=pd.read_csv(datadir + '/low.txt', sep='\n',header=None) 
  disgust=pd.read_csv(datadir + '/disgust.txt', sep='\n',header=None)
  happy=pd.read_csv(datadir + '/happy.txt', sep='\n',header=None)
  ...
  pn=pd.concat([happy,angry,disgust,low],ignore_index=True)

  print('happy count:' + str(len(happy)))
  print('angry count:' + str(len(angry)))
  print('disgust count:' + str(len(disgust)))
  print('low count:' + str(len(low)))
  return pn
...
def cnn_lstm(dict, x, y, xt, yt):
  ...
  print ("模型编译完成")
  model.fit(x, y, batch_size=lstm_batch_size, epochs=lstm_epochs, verbose=1)
  print ("模型训练完成")
  print ("保存模型")
        ...
        if len(line) <= 0:
          ...
          for s in proba:
            ...
            if s[3] == smax:
              index = 3
              des = u'低落'
            print('[' + str(test_count) + ']: ' + des + ' ' + str(s[index]) + ' ' + line[:-1])
    finally:
      f.close() # 确保关闭
...
if __name__=='__main__':
    ...
    if argvs_length >= 2:
        ...
        if action == 'train': # 训练
            ...
            sentence = " ".join(argvs[2:])
            predict(sentence)
        elif action == 'test':
            testfile = argvs[2]
            begin = datetime.datetime.now()
            result = predictarr(testfile)
            end = datetime.datetime.now()
            print("完成测试，数量", result)
